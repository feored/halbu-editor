...
<div class="skills-page container-fluid px-0">
	<SkillsHeader
		title={pageTitle}
	...
	{#if isSkillsContextLoading}
		<div class="alert alert-info py-2 px-3 mt-3 mb-0">Loading skills context...</div>
	{/if}

	<div class="skills-workspace row g-3 align-items-start mt-1 flex-nowrap">
		<div class="col skills-main-col">
			{#if pageNotices.length > 0}
				{#each pageNotices as notice}
					<div class={`alert py-2 px-3 ${notice.level === "warning" ? "alert-warning" : "alert-info"}`}>
						{notice.text}
					</div>
				{/each}
			{/if}
		...
		<div class="col-auto skills-inspector-col">
			<SkillInspectorPanel
